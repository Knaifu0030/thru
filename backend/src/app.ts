...
import { renderBrowserFixture, renderNadakacheriDemo, type MockPortal } from "./mock-portal.js";
import type { SkillRegistry } from "./registry.js";
import type { RunManager } from "./run-manager.js";
import { getWebcmdDiagnostic } from "./webcmd-diagnostic.js";

const METHODS = "GET, POST, DELETE, OPTIONS";
const HEADERS = "Content-Type, X-THRU-Admin-Key, X-Forge-Admin-Key, Prefer";
const MAX_BODY_BYTES = 1_048_576;

export interface AppDependencies { readonly registry: SkillRegistry; readonly executor: SkillExecutor; readonly mockPortal: MockPortal; readonly forgeEngine: THRUEngine; readonly runManager: RunManager }
...
function hasAdminAccess(req: IncomingMessage, config: AppConfig): boolean {
  ...
  const candidate = req.headers["x-thru-admin-key"] ?? req.headers["x-forge-admin-key"];
...
async function route(req: IncomingMessage, res: ServerResponse, config: AppConfig, dependencies: AppDependencies): Promise<void> {
  ...
  if (req.method === "GET" && url.pathname === "/health") {
    ...
    json(res, 200, { status: webcmd.status === "ready" ? "ok" : "degraded", service: "thru-backend", version: config.version, webcmd, skills: dependencies.registry.list().length }); return;
  }
  ...
  if (req.method === "GET" && (url.pathname === "/mock/nadakacheri/prepare" || url.pathname === "/mock/nadakacheri/status")) {
    const html = renderNadakacheriDemo(url.pathname.endsWith("status") ? "status" : "prepare");
    res.writeHead(200, { "Content-Type": "text/html; charset=utf-8", "Cache-Control": "no-store", "Content-Length": Buffer.byteLength(html) }); res.end(html); return;
  }
