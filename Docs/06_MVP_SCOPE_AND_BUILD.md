...
A local-first Forge where: a skill can be forged conversationally on a real website; the instant it exists it is usable as a button (Marketplace), an endpoint (REST), and a tool (MCP); skills self-heal when sites change; sensitive actions are human-gated everywhere. Shipped with 4–5 skills spanning categories so the marketplace reads as a marketplace.

**MVP acid test (all must be true before "done" — run against the PRODUCTION URLs, per `12_DEPLOYMENT.md`, not localhost):**
1. `forge new` (or the admin `/forge` endpoint) → real site → skill card appears on the LIVE Vercel Marketplace without restarting anything
...
5. `forge export` → `forge import` → skill runs — demoable locally; production persistence strategy is the image-bake approach in `12` §3

## The MUST / SHOULD / THEATER / WON'T ledger

**MUST (the acid test, decomposed):** Registry (validate/quarantine/atomic/events) · Forge engine (explore, narrate, infer schemas, confirm, save) · Executor (verify, envelope, vitals) · Healing rungs 1–2 · Gates (local pause+verify; gateway needs_human) · REST gateway w/ hot routes + input validation + CORS · MCP gateway w/ tool-list updates · Marketplace page (cards, search, three tabs, form-gen, forging state) · Mock site v1–v3 as internal routes + admin sabotage endpoint · 3 real skills + 1 mock skill · export/import · **production deploy: backend live on Azure Container Apps with min-replicas≥1, frontend live on Vercel, both reachable and CORS-connected** (see `12_DEPLOYMENT.md`)

**SHOULD (in descending value-per-hour):** Healing rung 3 (re-forge) · `healed_success` curl-loop demo polish · SSE narration streaming to the Use tab (else poll) · run queue w/ position feedback · `forge doctor` · 5th skill (one composition OR one more real site)
...
DEPLOY SKELETON (hello-world backend → Azure, hello-world frontend → Vercel, CORS proven)
  ...
  → forge engine v0 (manual-assist forging OK at first: agent explores, you confirm)
...
| 10:20–11:00 | Mock site as internal routes · registry · schemas · forge v0 + executor vs mock v1 (still redeployed to Azure as it's built, not left local) | — (watch, unblock) |
| 11:00–11:30 | **Forge first real skill (pnr)**, redeploy → ✅ **GATE 1: curl a real-site skill on the PRODUCTION Azure URL** — record it | Draft LinkedIn text + repo README skeleton (with URLs once known) |
...
| 12:15–13:00 | Healing r1–r2 + gates + `/admin/sabotage` endpoint → ✅ **GATE 3: curl-loop sabotage demo against production** — record | Record B-roll: forging narration, live Marketplace growing on Vercel |
