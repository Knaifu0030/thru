...
Local THRU · marketplace · REST+MCP gateway · self-healing · gates · 4–5 skills. Proof: the loop works and demos irresistibly.

### Phase 1 — The Sharable Registry (weeks)
`thru publish` → skills go to a shared cloud registry; `thru install <skill>` pulls them. Execution stays LOCAL (your browser, your sessions) — only the knowledge travels. This is the moment network effects switch on: every author makes every user's THRU more capable. Add: skill pages on the web, install counts (real now), author profiles, basic search/tags.
...
| Layer | Free | Paid |
|---|---|---|
| Local THRU (engine, your skills, your machine) | ✅ forever — this is distribution | — |
...
**What do you do?** THRU lets anyone teach an AI agent a website workflow once; it becomes a Skill — simultaneously a button, a REST API, and an MCP tool — published to a marketplace and self-healing when sites change. Agents connect once and can act on the API-less web.
...
**What have you built?** A working end-to-end MVP in a day: live teaching on real sites, three-surface consumption, an API that survived its website being vandalized on camera, and a permission layer that keeps humans on every sensitive action. (Link video.)
...
**Biggest risk?** (Say it before they do.) Site operators' tolerance of automation. Mitigations: human-speed local execution on the user's own sessions, gates on all side-effecting actions, terms-respect policy at teach time — and structurally, we're aligned with where the web must land as agents become normal users. Second risk: teaching quality on hostile/complex sites — the healing ladder plus human-in-the-loop teaching keeps the floor high while models raise the ceiling.
...
- [ ] Ship Phase-1 `thru publish/install` against a dead-simple hosted registry (even static hosting works)
- [ ] Get 3 outside users to teach one skill each; watch silently; fix the top 3 papercuts
