...
> "The web has a billion workflows and almost no APIs. And now a billion AI agents need to use them. Today, every team re-automates the same websites, badly, and every script dies the moment a site changes. We built THRU — where you teach a workflow once, and it becomes a skill anyone and any agent can use. Forever."

### Beat 1 — Cold Boot (0:25–0:40)
Marketplace on screen (the live Vercel URL, visible in the address bar): **empty**.
> "This is our marketplace — live in production right now, not running off my laptop. It's empty, because THRU knows nothing until it's taught. Let's teach it."

### Beat 2 — The First Skill (0:40–1:25) ▶
Terminal: `thru new` → goal + URL (the pre-tested real site). Narration streams; the "teaching…" card animates onto the shelf; agent proposes the contract; you confirm.
> "It explored the site, figured out the workflow, inferred the inputs and outputs — and asked when it wasn't sure. And now—" *(card flips live)* "—skill created."

### Beat 3 — Three Surfaces, One Skill (1:25–2:15) ▶ **the thesis moment**
> "Here's the point of THRU. That skill is already three things at once."
...
3. **AI tool:** switch to the external MCP client (Claude/other): "check PNR 852… for me" → it discovers and calls `thru_check_pnr` → answers.
> "For an agent: a tool it didn't have sixty seconds ago. One connection to THRU, and any agent can use the web like a human. Nothing was deployed. Nothing was restarted. We taught it — once."
...
> "Renamed the button, moved it, added a popup — the skill noticed, found the button by *meaning*, repaired itself, and versioned up. Callers never saw an error. This is running in production, right now — not a script on my laptop." *(If rung 3 is solid: sabotage v3 → re-teach beat: "and when the change is too big to patch, it re-teaches itself just the broken step.")*
...
> "Every skill here was taught, not programmed. Each one is a button, an API, and an AI tool — and they stay alive on their own. Built on webcmd: explore once, learn the workflow, reuse it forever. **THRU — where agents get their hands.**"
...
- Speeding up teaching footage 2× is fine (label it "2×"); splicing fake output is never fine.
...
Real site down at demo/record time?
 ├─ recon alternate up → thru/use that (skills are site-agnostic machinery)
