...
|---|---|---|---|
| **Teach** | Skill authors (power users, devs) | CLI (conversational) — later: web wizard | *teach* |
...
### J1 — Author teaches a skill (supply)
1. `thru new` → conversational prompt: URL + goal ("check PNR status; input: 10-digit PNR; output: train, date, passenger statuses")
...
4. Author confirms → **"⚒ Skill created: check-pnr v1"** → card appears on Marketplace, REST route registers hot, MCP tool list updates
...
Card → **API tab** → copy a request such as `curl -X POST http://localhost:8080/skills/check-pnr -H "Content-Type: application/json" -d '{"pnr":"8524..."}'` → typed JSON per the output schema. Errors are structured (`status: invalid_input | portal_error | needs_human | healed_success`). No SDK and no docs to read beyond the card.

### J4 — AI agent gains abilities (demand, the innovation)
Any MCP client adds the THRU Gateway URL once. Instantly, every marketplace skill is a tool with name + description + input schema. Agent reasons "I need a PNR status → I have a tool for that" and calls it. **A skill created at 10:14 is usable by every connected agent at 10:14.** Gated skills return `needs_human` with instructions rather than acting.
...
`thru export check-pnr` → one `.skill.json` file → send to anyone → `thru import` → on their marketplace, instantly. Skills are files; knowledge is portable. (Future: `thru publish` to a shared cloud registry — see `11`.)
...
- **THRU engine**: goal+URL → exploration → skill artifact; conversational narration; ambiguity questions; schema inference (inputs typed + validated, outputs structured)
...
- **Marketplace UI**: cards (name, description, site, stats, heal badge), search, Use/API/Agent tabs, live "teaching…" state, result rendering from output schema
...
- **Web-based teaching wizard** (remove the CLI from J1 → authors go mainstream)
- **Shared registry** (`thru publish`): the marketplace stops being local — network effects switch on
